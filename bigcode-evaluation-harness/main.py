...
from spy import SPECIAL_TOKENS
import datasets
import torch
import transformers
...
def parse_args():
    ...
    parser.add_argument(
        "--instruction_tokens",
        default=None,
        help="A series of instruction tokens used for instruction-tuning benchamrks separated by commae.g. <user_message>,<end_user_message>,<assistant_message>",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--tokenizer",
        type=str,
        default="Salesforce/codegen-350M-nl",
        help="Path/Name of the tokenizer to use",
    )
...
def main():
    ...
    if args.load_generations_path:
        ...
        root = args.load_generations_path
        evaluator = Evaluator(accelerator, None, None, args)
        for task in task_names:
            evaluator.args.load_generations_path = os.path.join(root, task, args.model.split('/')[-1], "generations.json")
        ...
        if args.modeltype == "causal":
            if args.model == 'gpt2':
                ...
                model = AutoModelForCausalLM.from_pretrained(
                    args.model if '/' in args.model else f"../checkpoints/100star/{args.model}/best_model",
                    cache_dir="../cached",
                    **model_kwargs,
                ).to(accelerator.device)
        ...
        if '/' not in args.model:
            if 'codegen' in args.model:
                tokenizer_name = 'Salesforce/codegen-350M-nl'
        ...
        # tokenizer_name = 'Salesforce/codegen-350M-nl' if '/' not in args.model else args.model
        ...
        for task in task_names:
            if args.generation_only:
                ...
                if accelerator.is_main_process:
                    save_dir = os.path.join(args.save_generations_path, task, args.model.split('/')[-1])
                    save_path = os.path.join(save_dir, "generations.json")
                    os.makedirs(save_dir, exist_ok=True)
                    with open(save_path, "w") as fp:
                        json.dump(generations, fp)
                        print(f"generations were saved at {save_path}")
                    if args.save_references:
                        with open(os.path.join(save_dir, 'references.json'), "w") as fp:
                            json.dump(references, fp)
                            print("references were saved")
            else:
                results[task] = evaluator.evaluate(task)
                results[task]["time"] = evaluator.time
                print(results[task])

    # Save all args to config
    results["config"] = vars(args)
    metric_save_dir = os.path.join(args.save_generations_path, task, args.model.split('/')[-1])
    os.makedirs(metric_save_dir, exist_ok=True)
    metric_save_path = os.path.join(metric_save_dir, "results.json")
