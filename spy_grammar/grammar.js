...
// tree-sitter generate ; tree-sitter test -f 'For statements'
const PREC = {
...
module.exports = grammar({
  ...
  rules: {
    ...
    print_statement: $ => choice(
      ...
      prec(100, prec.dynamic(100, seq(
        '<print>',
        spaceSep1(field('argument', $.expression)),
        optional(' '),
      ))),
    ),
    ...
    assert_statement: $ => seq(
      '<assert_stmt>',
      // comma
      spaceSep1($.expression),
    ),

    expression_statement: $ => choice(
      $.expression,
      // comma
      seq(spaceSep1($.expression), optional(' ')),
      $.assignment,
    ...
    match_statement: $ => seq(
      ...
      '<match_stmt>',
      // comma
      spaceSep1(field('subject', $.expression)),
      optional(' '),
    ...
    with_clause: $ => choice(
      seq(spaceSep1($.with_item), optional(' ')),
      // seq('(', spaceSep1($.with_item), optional(','), ')'),
    ),
    ...
    // The structure of this rule is different from the Python grammar because I didn't use parameters to remove the brackets.
    ...
    decorator: $ => seq(
      '@',
      $.expression,
      optional(choice('<line_sep>', $.comment)),
    ),
    ...
    dict_pattern: $ => seq(
      '{',
      optional(seq(
        spaceSep1(choice($._key_value_pattern, $.splat_pattern)),
        optional(' '),
      )),
      '}',
    ),
    ...
    binary_operator: $ => {
      const table = [
        ...
        [prec.left, '%', PREC.times],
        // [prec.left, '//', PREC.times],
        [prec.left, '<floordiv>', PREC.times],
        [prec.right, '<power>', PREC.power],
        [prec.left, '|', PREC.bitwise_or],
        [prec.left, '&', PREC.bitwise_and],
        [prec.left, '^', PREC.xor],
        [prec.left, '<lshift>', PREC.shift],
        [prec.left, '<rshift>', PREC.shift],
      ];
      ...
      repeat1(seq(
        field('operators',
          choice(
            ...
            '<lgne>',
            '<in>',
            alias(seq('<not>', '<in>'), '<notin>'),
            '<is>',
            alias(seq('<is>', '<not>'), '<isnot>'),
          )),
        $.primary_expression,
      )),
    )),
    ...
    lambda: $ => prec(PREC.lambda, seq(
      ...
      ':',
      field('body',prec.left(2,seq($.expression, optional('<line_sep>')))),
    )),
    ...
    augmented_assignment: $ => seq(
      ...
      // field('operator', choice(
        ...
        // '>>=', '<<=', '&=', '^=', '|=',
      // )),
      field('operator', choice('<augadd>', '<augsub>', '<augmul>', '<augdiv>', '<augmatmul>', '<augfloordiv>', '<augmod>', '<augpow>', '<augrshift>', '<auglshift>', '<augand>', '<augxor>', '<augor>')),
    ...
    subscript: $ => prec(PREC.call, seq(
      ...
      '[',
      // comma
      spaceSep1(field('subscript', choice($.expression, $.slice))),
      optional(' '),
      ']',
    )),
    ...
    dictionary: $ => seq(
      '{',
      optional(spaceSep1(choice($.pair, $.dictionary_splat))),
      optional(' '),
      '}',
    ),

    pair: $ => prec.dynamic(10, seq(
    ...
    for_in_clause: $ => prec(2, seq(
      optional('<line_sep>'),
      optional('<async_keyword>'),
      '<for>',
      field('left', $._left_hand_side),
      ' ',
      field('right', spaceSep1($._expression_within_for_in_clause)),
      optional(' '),
    )),
    ...
    conditional_expression: $ => prec.right(PREC.conditional, seq(
      ...
      optional('<line_sep>'),
      '<else>',
      prec.left(2,seq($.expression, optional('<line_sep>')))
    )),
    ...
    line_continuation: _ => token(seq('\\', '\0')),
